...
$followingposts = DB::query('SELECT posts.id, posts.body, posts.likes, usertable.username FROM usertable, posts, followers
...
<body>
    ...
    if ($showTimeline == false) {
        ...
        <?php 
        $imgposts = DB::query('SELECT imgtable.id, imgtable.img_url, imgtable.likes, imgtable.caption, usertable.username FROM imgtable, usertable, followers 
        ...
        foreach($imgposts as $p) {
            ...
            if (!DB::query('SELECT post_id FROM img_likes WHERE post_id=:postid AND user_id=:user_id', array(':postid'=>$p['id'], ':user_id'=>$user_id))) {
                $pic .= "<form style='display: inline;' action='index.php?type=img&postid=".$p['id']."' method='POST'>
                    <input class='likeunlike' type='image' src='profilepic/".$profilepic."' name='profile' value='profile'>
        ...
        foreach($followingposts as $post) {
            $p .= '<center><div class="text__post"><div class="padding">'.$post['body'].'<br> ~ <a href="/profile.php?username='.$post["username"].'"style="text-decoration: none;"> @'.$post['username'].'</a>';
            $p .= "<center><form style='display: inline;' action='index.php?postid=".$post['id']."&type=text' method='POST'></center>";
            ...
            $p .= "</center></form>
            <center><form action='index.php?postid=".$post['id']."' method='POST'>
                <textarea class='textarea__comment' name='commentbody' rows='2' cols='25'></textarea>
                <input class='button__comment' type='submit' name='comment' value='Comment'>
            </form></div></div></center>";
            $p .= "</br />";
            echo $p;
            echo "<center>".Comment::displayComments($post['id'])."</center>";
            $p = "";
        }
    } 
    ?>
